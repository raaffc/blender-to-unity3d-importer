...
public class EdysBlenderImporter : AssetPostprocessor{
    ...
    private void OnPostprocessModel(GameObject go){
        ...
        if (i1 < 0 || i2 < 0 || i1 >= i2){
            ...
            for (int i = 1, c = tokens.Length; i < c; i++){
                ...
                switch (token){
                    case "skipfix":
                        options.fixBlender = false;
                        break;
                    case "forcefix":
                        options.fixBlender = true;
                        break;
                    case "opt":
                        options.optimize = true;
                        break;
                    case "zreverse":
                        options.zReverse = true;
                        break;
                    case "noanimfix":
                        options.animFix = false;
                        break;
                    case "nofloatfix":
                        options.floatFix = false;
                        break;
                    case "nomods":
                        options.postMods = false;
                        break;
                    case "forcefixroot":
                        options.forceFixRoot = true;
                        break;
    ...
    */
    ...
    private static void OptimizeMeshInstancesMenu(){
        LogClear();
    ...
    private static void OptimizeMeshes(MeshFilter[] meshes){
        int unique;
        int instanced;
    ...
    private void ProcessBlenderObject(GameObject go){
    ...
    private void ProcessFirstLevel(GameObject go){
    ...
    private void ProcessChildren(GameObject go){
    ...
    private void ProcessAnimations(GameObject go){
    ...
    private bool IsMultipartObject(GameObject go){
    ...
    private void ApplyPostModifiers(GameObject go){
        string name = go.name.ToLowerInvariant();
        string options = "";
    ...
    private static void RotateMesh(GameObject go, Quaternion rot){
        Mesh mesh = null;
    ...
    private static void ConvertRotation(Transform t){
        Quaternion q = t.localRotation;
        t.localRotation = new Quaternion(-q.x, -q.z, q.y, -q.w);
    }

    private static void RotationFix180(Transform t){
        Quaternion q = t.localRotation;
        t.localRotation = new Quaternion(-q.x, q.y, -q.z, q.w);
    }

    private static void ConvertScale(Transform t){
    ...
    private static void FixFloatValues(Transform t, float threshold){
    ...
    private static Vector3 FixVector3(Vector3 v, float threshold){
        return new Vector3(FixFloat(v.x, threshold), FixFloat(v.y, threshold), FixFloat(v.z, threshold));
    }

    private static float FixFloat(float value, float threshold){
    ...
    private static void OptimizeMeshInstances(MeshFilter[] meshFilters, out int unique, out int instanced){
    ...
    private static uint CalculateMeshHash(Mesh mesh){
    ...
    private void ProcessClipRotation(AnimationClip clip, string objectPathName){
        ...
        if (curveY.length != keyframes || curveZ.length != keyframes || curveW.length != keyframes){
            LogInfo("WARNING: animated object [" + objectPathName + "] doesn't have the same keyframes for XYZW rotation curves. " + "x: " + curveX.length + " y:" + curveY.length + " z:" + curveZ.length + " w:" + curveW.length);
            return;
        }
    ...
    private void ProcessClipPosition(AnimationClip clip, string objectPathName){
        ...
        if (curveY.length != keyframes || curveZ.length != keyframes){
            LogInfo("WARNING: animated object [" + objectPathName + "] doesn't have the same keyframes for XYZ position curves. " + "x: " + curveX.length + " y:" + curveY.length + " z:" + curveZ.length);
            return;
        }
    ...
    private static void InvertTangents(ref Keyframe key){
        key.inTangent = -key.inTangent;
        key.outTangent = -key.outTangent;
    }

    private void ProcessClipScale(AnimationClip clip, string objectPathName){
        ...
        int keyframes = curveX.length;
        if (curveY.length != keyframes || curveZ.length != keyframes){
            LogInfo("WARNING: animated object [" + objectPathName + "] doesn't have the same keyframes for XYZ scale curves. " + "x: " + curveX.length + " y:" + curveY.length + " z:" + curveZ.length);
            return;
        }
    ...
    private static string m_debugInfo;
